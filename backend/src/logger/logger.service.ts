...
export class LoggerService {
   ...
   */
  private static getRequestUUID(): string {
  ...
  private getFileLogFormat(): Format {
    const addContext = winston.format(info => {
      const reqID = LoggerService.getRequestUUID()
  ...
  private setupConsoleStream() {
    this.logger.add(
      new winston.transports.Console({ format: this.getConsoleLogFormat() })
    )
    this.loggerHTTP.add(
      new winston.transports.Console({ format: this.getConsoleLogFormat() })
    )
